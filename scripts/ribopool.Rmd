...
trna_totcounts <- unlist(read.delim("../data/rna_align/counttable_tRNAs_new.txt.summary", 
                                    row.names = 1)[1,])
oligos_onlypeaks_totcounts <- unlist(read.delim("../data/rna_align/startpeaks_ctable.txt.summary", 
                                      row.names = 1)[1,])
barplot(oligos_totcounts)
barplot(oligos_onlypeaks_totcounts)
oligos_totcounts
```
...
for (i in 1:length(file.names.tot)) {
  ...
  print(g)
  print(sum(f[f$readlength %in% 15:45,]$count/ sum(f$count)))
  x[i] <- sum(f[f$readlength %in% 15:45,]$count/ sum(f$count))
}
...
dim(GenewiseCounts)
head(GenewiseCounts)
gwc_other <-  read.delim(
    "../data/rna_align/startpeaks_ctable.txt",
    sep = "\t", header = T, comment.char = "#")
```
...
gwc_tis <- gwc_other[,c(2,7:length(gwc_other[2,]))]
pnapat <- ".*.rna_align\\.(\\d\\d_.+_.+).bam"

colnames (gwc) <- gsub(pnapat,"\\1", colnames(gwc))
colnames (gwc_tis) <- gsub(pnapat,"\\1", colnames(gwc_tis))

head(gwc)
gwc$`73_RNA_input_control` <- NULL

rownames(gwc) <- gwc$Chr
rownames(gwc_tis) <- gwc_tis$Chr
...
compare how much % is in start region:
```{r}
gwc_starts <- gwc_other[,grepl("no|Chr", colnames(gwc_other))]
colnames (gwc_starts) <- gsub(pnapat,"\\1", colnames(gwc_starts))
gwc_all <- gwc[,grepl("no|Chr", colnames(gwc))]

acpP_starts <- gwc_starts %>% filter(grepl("acpP", Chr))
acpP_all <- gwc_all %>% filter(grepl("acpP", Chr))
mean(unlist(acpP_starts[-1]/acpP_all[-1]))
```
...
novel_sites$unique_lt <- paste0(novel_sites$locus_tag,"_", 
                                gsub("-", "_",novel_sites$start_position))
novel_sites$really_unique_lt <- paste0(novel_sites$locus_tag,"_", 
                                gsub("-", "_",gsub("-", "_", novel_sites$pos_from_annot_start)))

head(annot_sites)
head(novel_sites)

novel_sites$annot_peak_found_hoer <- novel_sites$locus_tag %in% annot_sites$locus_tag[annot_sites$peak_found_Hör=="True"]
novel_sites$annot_peak_found_meydan <- novel_sites$locus_tag %in% annot_sites$locus_tag[annot_sites$peak_found_meydan=="True"]
...
detected_types <- data.frame(unique_lt = annot_sites$unique_lt, 
                             ...
                             alt_site_detected_meydan = annot_sites$unique_lt %in% novel_sites$unique_lt[novel_sites$peak_found_meydan=="True"], 
                             annot_site_detected_meydan = as.logical(toupper(annot_sites$peak_found_meydan)),
                             riboseq_reads_detected = annot_sites$locus_tag %in% detected_riboseqgenes)

detected_types[detected_types$unique_lt %in% "b3843_4024988",4] <- FALSE
...
# TIS confirmed, no alternative TIS identified, meydan confirmed:
as_noalt_meydan <- length(detected_types$unique_lt[detected_types$annot_site_detected & !detected_types$alt_site_detected & detected_types$annot_site_detected_meydan])

# TIS confirmed, alternative TIS identified, meydan confirmed:
as_alt_meydan <- length(detected_types$unique_lt[detected_types$annot_site_detected & detected_types$alt_site_detected & detected_types$annot_site_detected_meydan & detected_types$riboseq_reads_detected])

#only in meydan annot
only_meydan_annot <- length(detected_types$unique_lt[!detected_types$annot_site_detected &
                                             !detected_types$alt_site_detected &
                                             detected_types$annot_site_detected_meydan &
                                             !detected_types$alt_site_detected_meydan &
                                             detected_types$riboseq_reads_detected])

allthree <- length(detected_types$unique_lt[detected_types$annot_site_detected &
                                             detected_types$alt_site_detected &
                                             detected_types$annot_site_detected_meydan &
                                             detected_types$alt_site_detected_meydan &
                                             detected_types$riboseq_reads_detected])
...
check venn mistake
```{r}
all = length(novel_sites$locus_tag[novel_sites$peak_found_Hör=="True" & novel_sites$peak_found_meydan=="True" & novel_sites$locus_tag %in% annot_sites$locus_tag[annot_sites$peak_found_Hör=="True"]& novel_sites$locus_tag %in% annot_sites$locus_tag[annot_sites$peak_found_meydan=="True"]])
...
make upset plot:
```{r}
rownames(detected_types) <- detected_types$unique_lt
detected_types <- detected_types[-1]
m1 <- make_comb_mat(detected_types[-5])
UpSet(m1)
eu <- euler(detected_types[-5], shape = "ellipse")
p1 <- plot(eu, quantities = eu$original.values)
venn <- plot(venn(detected_types[-5][c(1,4,2,3)]), fills=c("#FFFFFF",  "#99DBEF", "#6E6E6E", 
                                                   "#00A6D6"))

svg("../analysis/revision_06_2022/eulerplot_2.svg")
p1
dev.off()
svg("../analysis/revision_06_2022/venn_2.svg")
venn
dev.off()

write.csv(detected_types, "../analysis/revision_06_2022/venndiagram_with_meydan.csv")
x <- read.csv("../analysis/analysis_02_2022/table_detected_genes.csv")
sum(x$alt_site_detected & x$alt_site_detected_meydan)

s3 <- xlsx::read.xlsx("~/Downloads/Supplementary Table S3.xlsx", sheetIndex = 1)
gs3 <- s3$Locus.tag[s3$Peak.identified..this.study.==1 & s3$Peak.identified..Meydan.et.al...Mol.Cell..2019.]
gnew <- gsub("_.*", "",x$unique_lt[x$alt_site_detected & x$alt_site_detected_meydan & x$riboseq_reads_detected])
gnew[!gnew %in% gs3]
```

Investigate non-identified tis:
```{r}
lt_unannotated <- unique(gsub( "_.*", "", rownames(detected_types)[!detected_types$annot_site_detected]))
gff <- read.delim("../data/reference_sequences/oligos_cds_new.gff", sep="\t", header = F )
gff <- gff[gff$V3=="gene",]

gff$locustag <- ifelse(grepl(".*locus_tag=([^;\n]*).*", gff$V9), 
                       gsub(".*locus_tag=([^;\n]*).*", "\\1", gff$V9), "")

gff$typegene <- ifelse(grepl(".*gene_biotype=([^;\n]*).*", gff$V9),
                       gsub(".*gene_biotype=([^;\n]*).*", "\\1", gff$V9), "")

gff <- gff[gff$typegene!="",]

genetype_nontis <- sapply(lt_unannotated, function(x) {gff[gff$locustag==x,]$typegene[1]})
genetype_nontis<- genetype_nontis[!is.na(genetype_nontis)]

df_pie <- data.frame(type=c("protein coding", "pseudogene"), 
                     values = c(sum(genetype_nontis=="protein_coding"),                                                                         sum(genetype_nontis=="pseudogene")))
bp<- ggplot(df_pie, aes(x="", y=values, fill=type)) +  
...
pchart <- pie + scale_fill_manual(values = c("steelblue", "skyblue")) + blank_theme +
  theme(axis.text.x=element_blank())

svg("../analysis/revision_06_2022/piechart.svg")
pchart
dev.off()
